...
def fetch_fm_user_base_features(user: UserFeatureEntity) -> pd.DataFrame:
    # 1. 从数据库提取数据
...
# noinspection DuplicatedCode
def fetch_fm_spu_base_features(spu: SpuFeatureEntity) -> pd.DataFrame:
    # 1.1 提取所有物品对应的平均评分
...
def fetch_vector_pai_eas_data(resp_data):
    vectors = resp_data['vectors']
    version = resp_data['infos']['version']
    return vectors, version


# noinspection DuplicatedCode
class FMUserSideLocalPredictor(BasePredictor):

    def __init__(self, model_dir, model_version):
        super(FMUserSideLocalPredictor, self).__init__(model_dir, model_version)
        logger.info(f"开始加载恢复FM用户侧本地向量子模型:{model_dir}")
    ...
    def internal_predict(self, user: UserFeatureEntity) -> Tuple[List[float], str]:
        # 1. 获取当前用户对应的特征属性
        df: pd.DataFrame = fetch_fm_user_base_features(user)
    ...
    def predict(self, **kwargs) -> Optional[Tuple[Dict[str, Any], Any]]:
        ...
        if user is None:
            logger.warn("调用FM用户侧向量子模型必须传入UserFeatureEntity类型参数:user，当前为空。")
            return None
        if not isinstance(user, UserFeatureEntity):
            logger.warn("调用FM用户侧向量子模型必须传入UserFeatureEntity类型参数:user，当前类型不对。")
            return None
...
# noinspection DuplicatedCode
class FMSpuSideLocalPredictor(BasePredictor):
    def __init__(self, model_dir, model_version):
        super(FMSpuSideLocalPredictor, self).__init__(model_dir, model_version)
        logger.info(f"开始加载恢复FM商品侧本地向量子模型:{model_dir}")
        self.movie_id_mapping = SimpleMapping(os.path.join(model_dir, "dict", "movie_id.dict"))
    ...
    def internal_predict(self, spu: SpuFeatureEntity) -> Tuple[List[float], str]:
        # 1. 获取当前商品对应的特征属性
        df: pd.DataFrame = fetch_fm_spu_base_features(spu)
    ...
    def predict(self, **kwargs) -> Optional[Tuple[Dict[str, Any], Any]]:
        spu = kwargs.get('spu')
        if spu is None:
            logger.warn("调用FM商品侧向量子模型必须传入SpuFeatureEntity类型参数:spu，当前为空。")
            return None
...
# noinspection DuplicatedCode
class FMLocalPredictor(BasePredictor):
    def __init__(self, model_dir, model_version):
        super(FMLocalPredictor, self).__init__(model_dir, model_version)
        logger.info(f"开始加载恢复FM rank模型:{model_dir}")
