...
class GBDTLrLocalPredictor(BasePredictor):
    ...
    def _fetch_base_features(user: UserFeatureEntity, spus: Dict[int, SpuFeatureEntity]):
        ...
        for spu_id, spu in spus.items():
            ...
            data['max_rating_genre'] = data['max_rating_genre'] or 'action'
            data['max_rete_items_genre'] = data['max_rete_items_genre'] or 'action'
            # 合并当前商品特征
    ...
    def parse_dataset(self, xdf):
        ...
        for c in xdf.columns:
            if c in _columns:
                label_encoder = self.label_encoders[c]
                xdf[c] = label_encoder.transform(xdf[c].astype('str'))
            else:
    ...
    def internal_predict(self,
        ...
        # 2. 调用模型获取预测结果
        xdf = self.parse_dataset(df)
        xdf = self.gbdt.apply(xdf)
        xdf = np.reshape(xdf, (xdf.shape[0], -1))
        xdf = self.onehot.transform(xdf)
    ...
    def predict(self, **kwargs) -> Optional[Tuple[Dict[str, Any], Any]]:
        ...
        if (user is None) or (not isinstance(user, UserFeatureEntity)):
            logger.warn(f"调用Lr排序模型必须传入UserFeatureEntity类型参数:user，当前为:{user}")
            return None
        if (spus is None) or (not isinstance(spus, dict)) or (len(spus) == 0):
            logger.warn(f"调用Lr排序模型必须传入SpuFeatureEntity类型参数:spus，当前为:{spus}")
            return None
        # 调用模型
