...
def _user_predict_item_to_redis(trainset, algo):
    top_k = 100
    eval_predictions = []
    # with open('item_cf.txt', 'w', encoding="utf-8") as writer:
    ...
    with RedisUtil.get_redis() as redis:
        ...
        def save_to_redis():
            ...
            for _name, _value in tmp_records:
                _pipe.hset(_name, 'item_cf', _value)
...
def timed_scheduling():
    ...
    data_path = os.path.join(
        global_config.model_root_dir, "tmp", "item_cf", now.strftime("%Y%m%d_%H%M%S"), "training.txt"
    )
