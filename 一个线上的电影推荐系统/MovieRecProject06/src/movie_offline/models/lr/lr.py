...
class LrModel(object):
    ...
    def parse_dataset(self, xdf, onehot):
        ...
        xdf1 = onehot.fit_transform(xdf[_onehot_columns])
        # 删除做过onehot的列
        for c in _columns:
            if c in xdf:
                del xdf[c]
    ...
    def training(self, data_path):
        ...
        print(sklearn.metrics.confusion_matrix(y, y2_))
        ...
        # 模型保存
        now = datetime.now()
        model_version = f'lr_{now.strftime("%Y%m%d_%H%M%S")}'
        joblib.dump(
            {
                'onehot': onehot,
                'lr': lr,
...
def training(root_dir, output_dir):
    model = LrModel(model_dir=output_dir)
