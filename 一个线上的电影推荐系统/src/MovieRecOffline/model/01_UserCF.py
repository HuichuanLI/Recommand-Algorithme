...
def download_data(path):
    """
    从数据源(MySQL、Hive、HDFS等)通过代码将usercf模型训练需要的数据下载到本地
    :param path: 下载的路径
    :return:
    """
    pass
...
def training(path, k=100):
    """
    UserCF产生每个用户的推荐商品列表，并保存到redis中
    1. 构建模型UserCF
    2.
    :param path: 数据路径
    :param k: 每个用户产生的推荐商品列表最多的数目
    :return:
    """
    ...
    with RedisUtil._get_redis() as client:
        n_users = trainset.n_users  # 用户数目
        n_items = trainset.n_items  # 物品数目
        for iuid in tqdm(range(n_users)):
            uid = trainset.to_raw_uid(iuid)  # 将模型内部用户id转换为实际的用户id
            item_ratings = []
            # 提取当前用户已经有操作行为的商品列表(内部商品id列表)
            ...
            for iiid in range(n_items):
                ...
                rating = model.predict(uid, iid).est  # 预测实际用户对实际物品的评分
...
if __name__ == '__main__':
    _path = r"C:\Users\gerry_17578261252713\PycharmProjects\MovieRecSystem\data\u.data"
    download_data(_path)
    training(_path)
    shutil.rmtree(_path)
