...
class LocationNewBasedRecall(object):
    def __init__(self):
        self.REDIS_KEY = "rec:loc:new_spu:"
    ...
    def get_candidates(self, location_id: Optional[int] = None, number_push: Optional[int] = None) -> List[RecItem]:
        ...
        if location_id is None:
            return []
        _items = self._get_new_items(location_id)
    ...
    def _get_new_items(self, location_id: Optional[int] = None) -> List[RecItem]:
        """
        提取热销商品列表
        :param location_id: 位置id
        :return:
        ...
        result: List[RecItem] = []
        for _id in spu_ids:
            result.append(RecItem.get_recall_rec_item(_id, 1.0, "loc_news", "loc_news"))
        return result
