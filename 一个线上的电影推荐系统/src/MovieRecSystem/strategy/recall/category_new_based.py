...
class CategoryNewBasedRecall(object):
    def __init__(self):
        self.REDIS_KEY = "rec:cate:new_spu:"

    def get_candidates(self, category_id: Optional[int] = None, number_push: Optional[int] = None,
                       explain: str = "cate_news") -> List[RecItem]:
        """
        提取新商品作为推荐结果
        ...
        if category_id is None:
            return []
        _items = self._get_new_items(category_id, explain)
    ...
    def _get_new_items(self, category_id: Optional[int] = None, explain: str = "cate_news") -> List[RecItem]:
        """
        提取热销商品列表
        :param category_id: 品类id
        :param explain: 名称
