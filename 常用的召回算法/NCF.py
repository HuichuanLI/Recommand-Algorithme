...
def NCF(feature_columns, user_gmf_embedding_dim=20, item_gmf_embedding_dim=20,
    ...
    user_dim = 2 * user_gmf_embedding_dim
    item_dim = 3 * item_gmf_embedding_dim
    dim = (user_dim * item_dim) / (math.gcd(user_dim, item_dim))
    user_gmf_embedding_dim = int(dim / 2)
    item_gmf_embedding_dim = int(dim / 3)
    ...
    user_gmf_out = Lambda(lambda x: x, name="user_gmf_embedding")(user_vector_sum)
    item_gmf_out = Lambda(lambda x: x, name="item_gmf_embedding")(item_vector_sum)

    print(user_gmf_out, item_gmf_out)
    gmf_out = Multiply()([user_gmf_out, item_gmf_out])

    # Multi-Layer Perceptron (MLP) Part

    user_mlp_out = Lambda(lambda x: x, name="user_mlp_embedding")(user_vector_sum)

    item_mlp_out = Lambda(lambda x: x, name="item_mlp_embedding")(item_vector_sum)
