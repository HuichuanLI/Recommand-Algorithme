...
class CrossLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, is_train=False):
        x_0 = tf.expand_dims(inputs, axis=2)  # [batch, emb, 1]
        x_l = x_0
        for i in range(self.cross_layer_num):
            x1_w = tf.tensordot(x_l, self.kernels[i],
                                axes=(1, 0))  # [batch, emb, 1] * [emb, 1]
            dot_ = tf.matmul(x_0, x1_w)  # [batch, emb, 1] * [batch, 1, 1]
            x_l = dot_ + self.biases[i] + x_l
        out_ = tf.squeeze(x_l, axis=2)
        return out_
